<script lang="ts">
  import type { Effect, Shadow } from "@dashedhq/core";
  ...
  const effectLabel: Record<Effect["type"], string> = {
    blur: "Blur",
  ...
  function updateShadow(patch: Partial<Shadow>) {
    if (value.type === "drop-shadow" || value.type === "inner-shadow") {
      onValueChange({
        type: value.type,
...
<Popover.Root>
  ...
  >
    ...
    {:else if value.type === "drop-shadow" || value.type === "inner-shadow"}
      <div class="flex flex-col gap-2">
        ...
        <div class="grid grid-cols-2 gap-2">
          ...
          <NumberInput
            ...
            onValueChange={(v) => updateShadow({ spread: v })}
          >
            {#snippet startDecorator()}
